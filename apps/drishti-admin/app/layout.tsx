...
export default function RootLayout({
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      ...
      <body className={`${inter.variable} ${outfit.variable} antialiased flex flex-col md:flex-row h-screen overflow-hidden bg-[#030303] text-white selection:bg-[#f2ca50] selection:text-black`}>
        
        {/* Mobile Header */}
        <header className="flex md:hidden items-center justify-between px-6 py-4 bg-[#0a0a0a] border-b border-white/5 relative z-40">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-display text-base font-light tracking-widest text-[#f2ca50] uppercase leading-none">
              Drishti<span className="font-bold text-white block text-[8px] tracking-[0.3em] mt-0.5">Studios</span>
            </h1>
          </div>
          <button 
        ...
        <aside className={`fixed top-0 left-0 bottom-0 w-64 bg-[#0a0a0a] border-r border-white/5 z-50 md:hidden flex flex-col transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="flex-1 py-6 flex flex-col gap-2 px-4">
            {[
              ...
              >
                ...
                <span className={`text-sm font-medium ${pathname === link.href ? "text-white" : "text-[#8a8278] group-hover:text-white transition-colors"}`}>{link.label}</span>
              </Link>
            ))}
          </nav>
          ...
          <div className="p-4 border-t border-white/5">
            <button onClick={() => { setIsMobileMenuOpen(false); handleSignOut(); }} className="group flex items-center gap-4 px-4 py-3 w-full rounded-xl transition-all duration-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/20">
              ...
              <span className="text-sm text-[#8a8278] group-hover:text-red-400 font-medium">Sign Out</span>
            </button>
          </div>
        </aside>

        {/* Sidebar Navigation - Glassmorphic floating sidebar (Desktop) */}
        <aside 
          onMouseEnter={() => setIsCollapsed(false)}
          onMouseLeave={() => setIsCollapsed(true)}
          className={`${isCollapsed ? 'w-24' : 'w-72'} transition-all duration-500 ease-in-out m-4 rounded-3xl border border-white/5 bg-white/[0.02] backdrop-blur-3xl hidden md:flex flex-col h-[calc(100vh-32px)] shrink-0 relative overflow-visible shadow-[0_0_50px_rgba(0,0,0,0.5)] z-50`}
        >
          ...
          <nav className="flex-1 py-8 flex flex-col gap-3 px-6 relative z-10">
            {[
              ...
              <Link key={link.href} href={link.href} className="group relative flex items-center gap-4 px-4 py-3.5 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-0.5">
                ...
                <span className={`text-sm text-[#8a8278] group-hover:text-white font-medium transition-all duration-500 whitespace-nowrap overflow-hidden relative z-10 ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'} inline-block pointer-events-none`}>{link.label}</span>
              </Link>
            ))}
          </nav>
          ...
          <div className="p-6 border-t border-white/5 relative z-10">
            <button onClick={handleSignOut} className="group flex items-center gap-4 px-4 py-3 w-full rounded-2xl transition-all duration-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/20">
              ...
              <span className={`text-sm text-[#8a8278] group-hover:text-red-400 font-medium transition-all duration-500 whitespace-nowrap overflow-hidden relative z-10 ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'} inline-block pointer-events-none`}>Sign Out</span>
            </button>
          </div>
        </aside>
