  ...
  return (
    <html lang="en">
      ...
      <body className={`${inter.variable} ${outfit.variable} antialiased flex h-screen overflow-hidden bg-[#0a0a0a] text-[#e5e2e1] selection:bg-[#f2ca50] selection:text-black`}>
        ...
        {/* Sidebar Navigation - Dark UI */}
        <aside 
          onMouseEnter={() => setIsCollapsed(false)}
          onMouseLeave={() => setIsCollapsed(true)}
          className={`${isCollapsed ? 'w-24' : 'w-72'} transition-all duration-500 ease-in-out m-4 rounded-[2rem] border border-white/5 bg-[#0d0d0d] flex flex-col h-[calc(100vh-32px)] shrink-0 relative overflow-visible z-50`}
        >
          ...
          <nav className="flex-1 py-8 flex flex-col gap-3 px-6 relative z-10">
            {[
              ...
              <Link key={link.href} href={link.href} className="group relative flex items-center gap-4 px-4 py-3.5 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-0.5">
                ...
                <span className={`text-sm text-white/70 group-hover:text-white font-medium transition-all duration-500 whitespace-nowrap overflow-hidden relative z-10 ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>{link.label}</span>
              </Link>
            ))}
          </nav>
          ...
          <div className="p-6 border-t border-white/5 relative z-10">
            <button onClick={handleSignOut} className="group flex items-center gap-4 px-4 py-3 w-full rounded-2xl transition-all duration-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/20">
              ...
              <span className={`text-sm text-white/70 group-hover:text-red-400 font-medium transition-all duration-500 whitespace-nowrap overflow-hidden relative z-10 ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>Sign Out</span>
            </button>
          </div>
        </aside>

        {/* Main Content Area - The "Frame" */}
        <main className="flex-1 h-[calc(100vh-32px)] my-4 mr-4 rounded-[2rem] bg-[#0a0a0a] border border-white/5 relative overflow-y-auto">
